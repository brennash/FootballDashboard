...
class League:
	...
	def getHTMLTable(self):
		...
		html += '<th>GA</th>'
		html += '<th>Diff</th>'
		html += '<th>Points</th></tr></thead>\n'		
		html += '<tbody>\n'
		for team in self.leagueTable:
			print team.getTeamName()
			html += '<tr><td>{0}</td>'.format(team.getTeamName())
			html += '<td>{0}</td>'.format(team.getPlayed())
			html += '<td>{0}</td>'.format(team.getWins())
			html += '<td>{0}</td>'.format(team.getDraws())
			html += '<td>{0}</td>'.format(team.getLosses())
			html += '<td>{0}</td>'.format(team.getGF())
			html += '<td>{0}</td>'.format(team.getGA())
			html += '<td>{0}</td>'.format(team.getGoalDiff())
			html += '<td>{0}</td></tr>\n'.format(team.getPoints())
		html += '</tbody>\n'
		html += '</table>\n'
		return html
